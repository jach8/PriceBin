...
def optimize_and_simulate(stock, df, method='heston_path', verbose = False, **kwargs):
    """
    ...
    if method == 'heston_path':
        ...
        params.kappa, params.theta, params.v_0, params.rho, params.xi = optimized_params
        if verbose:
            print(f"Calibrated Heston parameters for {stock}: kappa={params.kappa:.4f}, theta={params.theta:.4f}, "
                f"v_0={params.v_0:.4f}, rho={params.rho:.4f}, xi={params.xi:.4f}")
...
#             print(f"Error occurred while processing {meth}: {e}")

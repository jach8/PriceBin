...
class anomaly_model(setup):
    ...
    def _evaluate_predictions(
        ...
        """
        Evaluate anomaly detection predictions using multiple metrics
        Args:
            y_true: True labels
        ...
        if hasattr(self.models[model_name], 'decision_function'):
            y_scores = self.models[model_name].decision_function(self.xtest)
            metrics['auc'] = roc_auc_score(y_true, -y_scores)  # Negative for correct orientation
    ...
    def _optimize_svm_params(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for _, row in top_3.iterrows():
                logger.info(f"Params: {row['params']}, Score: {row['mean_test_score']:.4f}")
    ...
    def _kernel_pca(self) -> KernelPCA:
        """
        Kernel PCA dimensionality reduction
        Returns:
            Fitted KernelPCA model
        """
        logger.debug("Running Kernel PCA...")
    ...
    def _kmeans_kernel_pca(self, threshold: float = 1.1618) -> None:
